...
public class PlayerAnimationController : MonoBehaviour
{
    ...
    private const string BLOCKATTACK = "BlockAttack";
    ...
    private void OnHit(bool dead, int arg1, float knockBack, Transform arg2)
    {
        if (dead){
            return;
        }
        movementAnim.SetTrigger(HIT);
    }
    ...
    private void Attack(float chargeAmount)
    {
        ...
        if (playerCombatController.IsBlocking)
        {
            currentWeaponAnimator.SetTrigger(BLOCKATTACK);
            return;
        }
